...
const PillNav = ({
  ...
  onLogout, // Props baru diterima di sini
}) => {
  ...
  // -- RENDER HELPER --
  const renderPillContent = (item, i, isActive) => (
    <>
      <span
        ...
        >
          ...
          >
            {items.map((item, i) => {
              ...
              // LOGIKA KHUSUS: Jika item Logout, render sebagai Button
              if (item.isLogout) {
                return (
                  <li key={item.href} role="none" className="flex h-full">
                    ...
                    >
                      {renderPillContent(item, i, false)}
                    </button>
                  </li>
                );
              }
      ...
      >
        <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
          {items.map((item) => {
            ...
            // Logic Mobile untuk Logout vs Link Biasa
            if (item.isLogout) {
              return (
                <li key={item.href}>
                  <button
                    className={`w-full text-left ${linkClasses}`}
                    style={{ ...defaultStyle, color: "#ff4d4d" }} // Merah sedikit untuk pembeda di mobile
                    onClick={() => {
                      onLogout();
